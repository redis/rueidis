...
`)
	...
	t.Run("Auth Credentials Refresh NoBg", func(t *testing.T) {
		n1, n2 := net.Pipe()
		mock := &redisMock{buf: bufio.NewReader(n2), conn: n2, t: t}
		refreshDone := make(chan struct{})
		go func() {
	...
	t.Run("Auth Credentials Refresh Waits For Busy Sync Pipe", func(t *testing.T) {
		n1, n2 := net.Pipe()
		mock := &redisMock{buf: bufio.NewReader(n2), conn: n2, t: t}
		...
		if calls.Load() != 1 {
			t.Fatalf("auth refresh should not fetch credentials while sync command is in flight, got %d calls", calls.Load())
		}
		close(replyGet)
		select {
		case resp := <-getResult:
		...
		case <-refreshDone:
		case <-time.After(time.Second):
			t.Fatal("auth refresh did not resume after sync command completed")
		}
		p.Close()
		mock.Close()
		n1.Close()
	...
	t.Run("Auth Credentials Refresh Closes Busy Pipe", func(t *testing.T) {
		n1, n2 := net.Pipe()
		mock := &redisMock{buf: bufio.NewReader(n2), conn: n2, t: t}
		...
		blockResult := make(chan RedisResult, 1)
		go func() {
			blockResult <- p.Do(context.Background(), cmds.NewBlockingCompleted([]string{"BLPOP", "k", "0"}))
		}()
		select {
		case <-blockRead:
		case <-time.After(time.Second):
			t.Fatal("blocking command was not sent")
		}
		deadline := time.After(time.Second)
		for p.Error() == nil {
			select {
			case <-deadline:
				t.Fatal("auth refresh did not close busy pipe")
		...
		case <-blockResult:
		case <-time.After(time.Second):
			t.Fatal("blocking command did not return after pipe close")
		}
		if calls.Load() != 1 {
			t.Fatalf("auth refresh should not fetch credentials while pipe is busy, got %d calls", calls.Load())
		}
		p.Close()
		mock.Close()
		n1.Close()
	...
	t.Run("Auth Credentials Refresh Rechecks Transaction After Fetching Credentials", func(t *testing.T) {
		n1, n2 := net.Pipe()
		mock := &redisMock{buf: bufio.NewReader(n2), conn: n2, t: t}
		...
		case <-refreshStarted:
		case <-time.After(time.Second):
			t.Fatal("auth refresh did not fetch credentials")
		}
		ExpectOK(t, p.Do(context.Background(), cmds.NewCompleted([]string{"MULTI"})))
		close(releaseAuth)
		time.Sleep(50 * time.Millisecond)
		...
		case <-refreshDone:
		case <-time.After(time.Second):
			t.Fatal("auth refresh did not resume after transaction closed")
		}
		p.Close()
		mock.Close()
		n1.Close()
	...
	t.Run("Auth Credentials Refresh Waits For Open Transaction", func(t *testing.T) {
		n1, n2 := net.Pipe()
		mock := &redisMock{buf: bufio.NewReader(n2), conn: n2, t: t}
		refreshDone := make(chan struct{})
		go func() {
		...
		time.Sleep(50 * time.Millisecond)
		if calls.Load() != 1 {
			t.Fatalf("auth refresh should not fetch credentials while transaction is open, got %d calls", calls.Load())
		}
		if err := p.Do(context.Background(), cmds.NewCompleted([]string{"EXEC"})).Error(); err != nil {
			t.Fatalf("EXEC failed: %v", err)
		}
	...
	t.Run("Auth Credentials Refresh Tracks DoMulti Transactions", func(t *testing.T) {
		n1, n2 := net.Pipe()
		mock := &redisMock{buf: bufio.NewReader(n2), conn: n2, t: t}
		refreshDone := make(chan struct{})
		go func() {
		...
		for _, resp := range p.DoMulti(context.Background(), cmds.NewCompleted([]string{"MULTI"})).s {
			ExpectOK(t, resp)
		}
		time.Sleep(50 * time.Millisecond)
		if calls.Load() != 1 {
			t.Fatalf("auth refresh should not fetch credentials while DoMulti transaction is open, got %d calls", calls.Load())
		}
		...
		case <-refreshDone:
		case <-time.After(time.Second):
			t.Fatal("auth refresh did not resume after DoMulti transaction closed")
		}
		p.Close()
		mock.Close()
		n1.Close()
	...
	t.Run("Auth Credentials Refresh Waits For In Flight Do Multi", func(t *testing.T) {
		n1, n2 := net.Pipe()
		mock := &redisMock{buf: bufio.NewReader(n2), conn: n2, t: t}
		...
		go func() {
			multiResult <- p.Do(context.Background(), cmds.NewCompleted([]string{"MULTI"}))
		}()
		select {
		case <-multiRead:
		case <-time.After(time.Second):
		...
		time.Sleep(50 * time.Millisecond)
		if calls.Load() != 1 {
			t.Fatalf("auth refresh should not fetch credentials while MULTI reply is in flight, got %d calls", calls.Load())
		}
		close(replyMulti)
		select {
		...
		case <-refreshDone:
		case <-time.After(time.Second):
			t.Fatal("auth refresh did not resume after in-flight transaction closed")
		}
		p.Close()
		mock.Close()
		n1.Close()
	...
	t.Run("Auth Credentials Refresh Waits For In Flight DoMulti Multi", func(t *testing.T) {
		n1, n2 := net.Pipe()
		mock := &redisMock{buf: bufio.NewReader(n2), conn: n2, t: t}
		multiRead := make(chan struct{})
		replyMulti := make(chan struct{})
		...
		go func() {
			multiResult <- p.DoMulti(context.Background(), cmds.NewCompleted([]string{"MULTI"}))
		}()
		select {
		case <-multiRead:
		case <-time.After(time.Second):
		...
		time.Sleep(50 * time.Millisecond)
		if calls.Load() != 1 {
			t.Fatalf("auth refresh should not fetch credentials while DoMulti MULTI reply is in flight, got %d calls", calls.Load())
		}
		close(replyMulti)
		select {
		case resp := <-multiResult:
		...
		case <-refreshDone:
		case <-time.After(time.Second):
			t.Fatal("auth refresh did not resume after in-flight DoMulti transaction closed")
		}
		p.Close()
		mock.Close()
		n1.Close()
	...
	t.Run("Auth Credentials Refresh After StopTimer", func(t *testing.T) {
		...
		case <-refreshDone:
		case <-time.After(time.Second):
			t.Fatal("auth refresh stopped with StopTimer")
		}
		p.Close()
		mock.Close()
		n1.Close()
...
func TestClientSideCachingStaticClientTTL(t *testing.T) {
	...
	invalidate := func(keys RedisMessage) {
		mock.Expect().Reply(slicemsg(
			'>',
			[]RedisMessage{
				strmsg('+', "invalidate"),
	...
	invalidate(slicemsg('*', []RedisMessage{strmsg('+', "a")}))
...
// TestClientSideCachingStaticClientTTLNilReply verifies that a Redis nil
...
func TestClientSideCachingStaticClientTTLNilReply(t *testing.T) {
	...
	go func() {
		mock.Expect("CLIENT", "CACHING", "YES").
			Expect("GET", "absent").
			ReplyString("OK").
			Reply(RedisMessage{typ: typeNull})
	}()
...
// cmd (resp.s[1].err non-nil — distinct from a wire-level RedisError). The
